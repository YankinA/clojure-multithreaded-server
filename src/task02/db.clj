...
(defn load-initial-data []
  ...
  (reset! student (conj (->> (data-table (csv/read-csv (slurp "student.csv")))
...
(defn delete [data & {:keys [where]}]
  (if (fn? where)
    (reset! data (vec (remove where @data)))
    (reset! data [])))
...
(defn update [data upd-map & {:keys [where]}]
  (reset! data (mapv (fn [item] (if (fn? where)
                                  (if (where item) (merge item upd-map) item)
                                  (merge item upd-map)))
                     @data)))
...
(defn insert [data new-entry]
  (reset! data (conj @data new-entry)))
